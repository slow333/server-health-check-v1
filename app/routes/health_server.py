...
bp = Blueprint("servers", __name__, url_prefix="/health/servers")

@bp.route("/")
def index():
    pagination_data = pagenation(Server)
    current_user_id = current_user.get_id()
    server_list_by_user = db.session.query(Server).join(Server.operators).filter(Users.id == current_user_id).all()
...
def create_server():
    ...
    if request.method == "POST":
        ...
        if not ip_addr or not login_id or not server_name:
            flash("IP 주소, 로그인 ID, 서버 이름은 필수입니다.")
            return render(url_for("servers.edit_server"))
        query_existing = db.session.query(Server).filter_by(ip_addr=ip_addr).first()
        if query_existing:
            flash("이미 존재하는 서버입니다.")
            return render(url_for("servers.create_server"))

        new_server = Server(server_name=server_name, login_id=login_id, ip_addr=ip_addr, port=port, password=password)
